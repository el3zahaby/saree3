                    ...
                    <a href="{{url('authorProfile')}}">
                        <div class="outer-ring">
                            ...
                            <figure class="user-avatar">
                                <img src="{{ asset('images/avatars/'.user()->image) }}" alt="avatar"
                                     style="height: 100%">
                            </figure>
                        </div>
                    </a>
            ...
            <!-- ACCOUNT ACTIONS -->
                <div class="account-actions">
                    @if(!auth()->guard()->check())
                        ...
                        <a href="#login-form" class="button secondary open-login-popup"> <i class="fa fa-sign-in"></i>
                            تسجيل الدخول </a>
                        ...
                        <div id="login-form" class="form-popup heddin-form">
                            ...
                            <div class="form-popup-content" style="overflow: hidden;">
                                ...
                                <!-- /LINE SEPARATOR -->
                                <form id="main-login-form" action="{{ route('login') }}" method="POST"
                                      style="direction: rtl;text-align: right;">
                                @csrf

                                <!-- Input Email -->
                                    <label for="email" class="rl-label">البريد الإلكتروني</label>
                                    <input type="email" name="email" style="margin-bottom: 8px;" id="email"
                                           class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}"
                                           placeholder="إدخل بريدك الإلكتروني ..." value="{{ old('email') }}" required
                                           autofocus>
                                    @if ($errors->has('email'))
                                ...
                                <!-- CHECKBOX -->
                                    ...
                                    <p style="text-align: left;">هل نسيت كلمة المرور ؟ <a href="#" class="primary">!إضغط
                                            هنا</a></p>
                                        ...
                                        @forelse(check_sub_section($value->section_id) as $keyitem => $valueitem)
                                            <!-- FEATURE LIST ITEM -->
                                                <li class="dropdown-item feature-list-item" style="position:relative;">
                                                    <a id="g"
                                                       href="#">{!! json_decode($valueitem->section_title)->ar !!}</a>
                                                    <!-- DROPDOWN -->
...
<div class="survey xmalert alert-box" style="visibility: visible; opacity: 1;display: none; position: fixed; z-index: 100000; transition: all 0.3s ease-in-out 0s; top: auto; bottom: 30px; left: auto; right: 30px;">							<figure class="survey-img">								<img src="website/images/dashboard/alert-logo.png" alt="survey-img">							</figure>							<p class="text-header">مرحباً بك على منصة سريع</p>							<p class="info">منصة سريع هي أكبر منصة عربية للمنتجات التقنية و الخدماتا الإلكترونية و العديد من الخدمات المميزة </p>							<p class="timestamp"></p>
    <a href="alerts-notifications.html" class="button mid dark"> تفقد خدماتنا <span class="primary"> الان </span></a>
    <img class="close-btn" src="{{ asset('website/images/dashboard/notif-close-icon.png') }}" alt="close-icon">
</div>
<!-- jQuery -->
<script src="{{ url('website/js/vendor/jquery-3.1.0.min.js') }}"></script>
<!-- Tooltipster -->
...
<script>
    $(document).ready(function () {
        var alerted = localStorage.getItem('alerto') || '';
        console.log(alerted);
        if (alerted == 'yes') {
        ...
        localStorage.setItem('alerto','yes');
